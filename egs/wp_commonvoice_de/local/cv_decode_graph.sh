...
test=${langdir}_test_bd_tgpr
fsttablecompose ${langdir}_larger/L.fst $test/G.fst | fstdeterminizestar --use-log=true | \
  fstminimizeencoded | fstarcsort --sort_type=ilabel > $tmpdir/LG.fst || exit 1;
fsttablecompose ${langdir}_larger/T.fst $tmpdir/LG.fst > $test/TLG.fst || exit 1;

# test=${langdir}_test_bd_tg
# fsttablecompose ${langdir}_larger/L.fst $test/G.fst | fstdeterminizestar --use-log=true | \
#   fstminimizeencoded | fstarcsort --sort_type=ilabel > $tmpdir/LG.fst || exit 1;
# fsttablecompose ${langdir}_larger/T.fst $tmpdir/LG.fst > $test/TLG.fst || exit 1;
