...
mkdir -p $dir/scoring_kaldi_wer_from_bpe
cat $data/text | $ref_filtering_cmd > $dir/scoring_kaldi/test_raw_filt.txt || exit 1;
for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
    mkdir -p $dir/scoring_kaldi/penalty_wer_$wip/log
    for w in `seq $min_lmwt $max_lmwt`; do
        python local/bpe2wrd.py ${dir}/scoring_kaldi/penalty_${wip}/${w}.txt $dir/scoring_kaldi/penalty_wer_${wip}/${w}.txt
    done
    $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring_kaldi/penalty_wer_$wip/log/score.LMWT.log \
      cat $dir/scoring_kaldi/penalty_wer_$wip/LMWT.txt \| \
      compute-wer --text --mode=present \
      ark:$dir/scoring_kaldi/test_raw_filt.txt  ark,p:- ">&" $dir/wer_from_bpe_LMWT_$wip || exit 1;
done

for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
    for lmwt in $(seq $min_lmwt $max_lmwt); do
    # adding /dev/null to the command list below forces grep to output the filename
        grep WER $dir/wer_from_bpe_${lmwt}_${wip} /dev/null
    done
done | utils/best_wer.sh  >& $dir/scoring_kaldi/best_wer_from_bpe || exit 1
