...
nj=20
data_de=CommonVoice/cv-corpus-5.1-2020-06-22/de
...
if [ $stage -le 7 ] && [ $stop_stage -ge 7 ]; then
    for set in test_de; do
        ...
        input_scp=$(readlink -f data/all_ark/test.scp)
        cd $CURDIR
        python3 -m scripts.inference            \
        --resume=$STNAS/ckpt/infer.pt           \
        --netconfig=$STNAS/config.json          \
        --nj=$nj                                \
        --input_scp=$input_scp                  \
        --output_dir=$ark_dir                   \
        || exit 1
        ...
        ctc-crf/decode.sh  --stage 1 \
            ...
            data/lang_phn_test data/$set $input_scp $dir/decode_${set} || exit 1
        ...
        grep WER $dir/decode_${set}*/cer_* | utils/best_wer.sh
    done
fi
